...
var intToBytes = function(x, bytes){
    var i = 4;
    while (i){
        i --;
        bytes[i] = x % 256;
...
var randomBytes = function(size, res){
    while (size > 0){
        size --;
...
var hexFromBytes = aes.utils.hex.fromBytes;
var hexToBytes = aes.utils.hex.toBytes;
var utf8FromBytes = aes.utils.utf8.fromBytes;
var utf8ToBytes = aes.utils.utf8.toBytes;
...
// this method will convert string to spec bytes
var strToBytes

var decryptHexToObj = function(hex, cr){
    hex = hexToBytes(hex);
    if (hex.length == 0) {
        return {};
    }
    hex = cr.decrypt(hex);
    var len = bytesToInt(hex); 
    var obj = hex.slice(4, len+4);
    obj = utf8FromBytes(obj);
    obj = JSON.parse(obj);
    return obj;
}

var encryptObjToHex = function(obj, cr){
    // convert object to string
    var obj = JSON.stringify(obj);
    // convert string to bytes
    obj = utf8ToBytes(obj);
    var len = obj.length;
    var ret = intToBytes(len, [])
    ret.push(...obj);
    len += 4;
...
aes.decryptHexToObj = decryptHexToObj;

aes.utils.randomInt = randomInt;
aes.utils.randomBytes = randomBytes;
aes.utils.randomHex = randomHex;
aes.utils.intToBytes = intToBytes;
aes.utils.bytesToInt = bytesToInt;
...
var encrypt = root.aesjs.encryptObjToHex;
var decrypt = root.aesjs.decryptHexToObj;
...
var postjson = function(i, obj, ft){
    ...
    try{
        ...
        ft.cr = new aesjs.ModeOfOperation.ecb(
            aesjs.utils.hex.toBytes(msg).slice(0,32));
        ft.id = xhr.response;
...
fr.addEventListener("load", function() {
    ...
    var id = aesjs.utils.randomHex(8);
    var crypto = new aesjs.ModeOfOperation.ecb(cr);
